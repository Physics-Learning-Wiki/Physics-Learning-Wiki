...
计算其伴随：$\hat{U}^\dagger(t) = \exp\left(+\dfrac{i}{\hbar}\hat{H}^\dagger t\right) = \exp\left(+\dfrac{i}{\hbar}\hat{H}t\right) = \hat{U}^{-1}(t)$．
$\hat{U}(t)$ 严格为酉算符！它保证了薛定谔波动演化中全空间波函数归一化永远保持为 1．
