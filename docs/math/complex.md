...
**例题：** 计算 $i^i$．**解：** 首先，将 $i$ 写成极坐标形式．$i$ 的模是 1，辐角是 $\pi/2$．
所以 $i = e^{i\pi/2}$．
...
**例题：** 判断函数 $f(z) = z^2$ 是否为解析函数．**解：** $f(z) = (x+iy)^2 = (x^2 - y^2) + i(2xy)$．
...
**例题：** 计算积分 $\oint_C \frac{e^z}{z^2} dz$，其中 $C$ 是包围原点的任意简单闭合路径．**解：** 被积函数 $f(z) = \frac{e^z}{z^2}$ 在 $z=0$ 处有一个奇点．
为了求留数，我们将 $e^z$ 在 $z=0$ 附近展开为泰勒级数：$e^z = 1 + z + \frac{z^2}{2!} + \dots$ 所以，$f(z) = \frac{1}{z^2}(1 + z + \frac{z^2}{2!} + \dots) = \frac{1}{z^2} + \frac{1}{z} + \frac{1}{2} + \dots$ 根据定义，留数是 $(z-0)^{-1}$ 即 $1/z$ 项的系数，所以 $\text{Res}(f, 0) = 1$．
根据留数定理：
