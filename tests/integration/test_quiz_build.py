...
def build_site(tmp_path: Path, *, preview: bool = False) -> Path:
    ...
    subprocess.run(
        [sys.executable, "-m", "mkdocs", "build", "--clean", "--site-dir", str(destination)],
        cwd=ROOT,
        env=environment,
        check=True,
        capture_output=True,
...
def test_production_build_contains_construction_cards_and_no_drafts(tmp_path: Path) -> None:
    site = build_site(tmp_path)
    assert (site / "quiz" / "index.html").exists()
    manifest = json.loads((site / "_generated" / "question-bank" / "manifest.json").read_text(encoding="utf-8"))
    ...
    newton = (site / "mechanics" / "dynamics" / "newton-laws" / "index.html").read_text(encoding="utf-8")
    ...
    assert "mech-kin-linear-0001" not in (site / "search" / "search_index.json").read_text(encoding="utf-8")
